...
class Filmoteka {
    constructor() {
        ...
        this.ui = new FilmotekaUI({
            ...
            onSourceSwitch: (filmId) => this.getPrimarySource(filmId)
        });
    ...
    async watchMovie(filmId, title, year) {
        this.ui.showPagination(false);
        ...
        try {
            const iframeUrl = await this.api.getIframeUrl(filmId);
            this.ui.showMovieFrame(iframeUrl, filmId);
        } catch (error) {
            const backupUrl = `https://ddbb.lol/?id=${filmId}&n=0`;
            this.ui.showMovieFrame(backupUrl, filmId);
        }
    ...
    async getPrimarySource(filmId) {
        return await this.api.getIframeUrl(filmId);
    }
